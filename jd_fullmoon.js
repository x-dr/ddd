...
月满金秋 佳节聚惠
2021-9-18 ~ 2021-9-28
https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/5077990?activityId=21h3i1h3h2u1h3g43gy4g3yu2u23u423
*/
...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/${$.authorNum}?activityId=${$.activityId}&shareUuid=7106dda69bcf4423807dab964a6f9c16&adsource=null&shareuserid4minipg=null&shopid=${$.activityShopId}&lng=00.000000&lat=00.000000&sid=&un_area=`
            await marry();
            await $.wait(3000)
            if ($.bean > 0) {
...
async function marry() {
    ...
    if ($.token) {
        ...
        if ($.secretPin) {
            ...
            if ($.index === 1) {
                await task('dz/openCard/activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=7106dda69bcf4423807dab964a6f9c16`, 0, 1)
            } else {
                await task('dz/openCard/activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=7106dda69bcf4423807dab964a6f9c16`)
            }
            await task('dz/openCard/checkOpenCard', `activityId=${$.activityId}&actorUuid=${$.actorUuid}&shareUuid=7106dda69bcf4423807dab964a6f9c16&pin=${encodeURIComponent($.secretPin)}`)
            $.log("->关注店铺")
            if ($.shopTask) {
